...
function buildUserPrompt(input: AnalyzeAuditLogInput): string {
...
${input.entries
  .map(
    (e) =>
      `- [${e.created_at}] ${e.action} → ${e.target_type}${e.target_label ? ` (${e.target_label})` : ''}${e.details ? ` — ${e.details}` : ''}`,
  )
  .join('\n')}`;
}
...
export async function analyzeAuditLog(
  ...
  try {
    ...
    return JSON.parse(response.content) as AnalyzeAuditLogOutput;
  } catch (err) {
    console.warn(
      '[AI] analyzeAuditLog failed:',
