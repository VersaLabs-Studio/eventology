...
function buildUserPrompt(input: GenerateEventDescriptionInput): string {
...
): Promise<GenerateEventDescriptionOutput | null> {
  try {
    ...
    const result = JSON.parse(response.content) as GenerateEventDescriptionOutput;
    ...
    console.warn(
      '[AI] generateEventDescription failed:',
