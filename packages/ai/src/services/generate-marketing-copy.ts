...
function buildUserPrompt(input: GenerateMarketingCopyInput): string {
...
): Promise<GenerateMarketingCopyOutput | null> {
  try {
    ...
    const result = JSON.parse(response.content) as GenerateMarketingCopyOutput;
    ...
    console.warn(
      '[AI] generateMarketingCopy failed:',
