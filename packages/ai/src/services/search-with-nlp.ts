...
function buildUserPrompt(input: SearchWithNLPInput): string {
 ...
 */
export async function searchWithNLP(
  ...
  try {
    ...
    return JSON.parse(response.content) as SearchWithNLPOutput;
  } catch (err) {
    console.warn(
      '[AI] searchWithNLP failed:',
