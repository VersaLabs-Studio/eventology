...
function buildMessages(input) {
    ...
    if (input.context) {
        ...
        const parts = [];
        if (ctx.user_name)
            parts.push(`User name: ${ctx.user_name}`);
        if (ctx.user_role)
            parts.push(`User role: ${ctx.user_role}`);
        if (ctx.current_page)
            parts.push(`Currently on: ${ctx.current_page}`);
        if (ctx.event_title)
            parts.push(`Viewing event: ${ctx.event_title}`);
        if (parts.length > 0) {
            contextSuffix = `\n\nContext:\n${parts.join('\n')}`;
        }
    }
 ...
 */
export async function chatbotResponse(input) {
    try {
        ...
        const escalate_to_human = escalateKeywords.some((kw) => content.toLowerCase().includes(kw));
        return {
            response: content,
            escalate_to_human,
        };
        ...
        console.warn('[AI] chatbotResponse failed:', err instanceof Error ? err.message : 'Unknown error');
        return null;
    }
}
//# sourceMappingURL=chatbot-response.js.map