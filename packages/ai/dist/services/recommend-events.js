...
${input.available_events
        .map((e) => `- [${e.id}] ${e.title} | ${e.category} | ${e.event_type} | ${e.start_date}${e.price !== undefined ? ` | ${e.price} ETB` : ''}`)
        .join('\n')}
...
export async function recommendEvents(input) {
    try {
        ...
        console.warn('[AI] recommendEvents failed:', err instanceof Error ? err.message : 'Unknown error');
        return null;
    }
}
//# sourceMappingURL=recommend-events.js.map